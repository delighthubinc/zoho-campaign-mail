...
- 新規セミナーは `template_type: seminar` を使用し、`preset` は `standard` / `large` の初期構成案としてユーザー確認を得る。最終表示と順序は `blocks` を正とし、preset選択後もcampaign単位で追加・削除・並び替えできる。既存の `large_seminar` 等は後方互換用として変更しない。
...
- CTAとUTMの正式ルールは `docs/MAIL_GENERATION_RULES.md` に従う。ChatGPTはcampaignごとにUTM候補を提示し、ユーザーの明示確認後、HTMLプレビュー時点から本番と同じCTA URLを使用する。
